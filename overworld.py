...
from decoration import Nite

class Node(pygame.sprite.Sprite):
	def __init__(self,pos,status,icon_speed,path):
		super().__init__()
		self.frames = import_folder2(path)
		self.frame_index = 0
...
class Icon(pygame.sprite.Sprite):
	def __init__(self,pos):
		super().__init__()
		self.pos = pos
		self.image = pygame.image.load('images/overworld/owhobbit.png').convert_alpha()
...
class Overworld:
	...
	def draw_paths(self):
		if self.max_level > 0:
			...
			pygame.draw.lines(self.display_surface,'#849373',False,points,6)
	...
	def update_icon_pos(self):
		if self.moving and self.move_direction:
			...
			if target_node.detect_zone.collidepoint(self.icon.sprite.pos):
				self.moving = False
				self.move_direction = pygame.math.Vector2(0,0)
	...
	def run(self):
		...
		self.nite.draw(self.display_surface)
		self.draw_paths()
